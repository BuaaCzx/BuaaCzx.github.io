...
对于第二部分，我们不难发现，如果一对朋友在 $k$ 次远足中总共被选择了 $x$ 次，那么它将会产生 $0 + 1 + ... + (x-1) = \dfrac{x(x-1)}{2}$ 的贡献，而这对朋友在 $k$ 次远足中被选择 $x$ 次的概率为 ${C_k^x} ({\dfrac{1}{S}})^x (\dfrac{S-1}{S})^{k - x}$。因此，每对朋友产生的期望值为 ${C_k^x} ({\dfrac{1}{S}})^x (\dfrac{S-1}{S})^{k - x} \dfrac{x(x-1)}{2}$，共有 $m$ 对朋友，将这个期望值乘上 $m$，即为总的期望值。
